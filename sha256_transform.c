...
static void			sha256_transform_help(t_sha256 *ctx,\
t_abcd256 abcd256, unsigned int m[64])
{
	unsigned int	t1;
	unsigned int	t2;
	int				i;
	...
	while (++i < 64)
	{
		t1 = abcd256.h + EP1(abcd256.e) +\
		CH(abcd256.e, abcd256.f, abcd256.g) + g_k[i] + m[i];
...
static void			sha256_transform_help_2(unsigned int *m,\
